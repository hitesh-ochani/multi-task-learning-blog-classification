...
class RCNN_MTL(nn.Module):
	def __init__(self, batch_size, output_size,output_size2, hidden_size, vocab_size, embedding_length, weights):
		...
		self.label2 = nn.Linear(hidden_size, output_size2)
	def forward(self, input_sentence, batch_size=None):
		...
		if batch_size is None:
			h_0 = Variable(torch.zeros(2, self.batch_size, self.hidden_size).to('cpu')) #.cuda()) # Initial hidden state of the LSTM
			c_0 = Variable(torch.zeros(2, self.batch_size, self.hidden_size).to('cpu')) #.cuda()) # Initial cell state of the LSTM
		else:
			h_0 = Variable(torch.zeros(2, batch_size, self.hidden_size).to('cpu')) #.cuda())
			c_0 = Variable(torch.zeros(2, batch_size, self.hidden_size).to('cpu')) #.cuda())
		...
		y = y.squeeze(2)
		logits = self.label(F.relu(self.hidden1(y)))
		logits2 = self.label2(F.relu(self.hidden2(y)))
